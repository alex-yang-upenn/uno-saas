...
const EditorCanvasSidebar = ({ nodes }: Props) => {
  ...
  const { googleFile, setSlackChannels } = useUnoStore()
  ...
  return (
    <aside>
      <Tabs defaultValue="actions" className="h-screen overflow-scroll pb-48">
        ...
        <TabsContent value="actions" className="flex flex-col gap-4 p-4">
          {Object.entries(EditorCanvasDefaultCardTypes).filter(
            ([_, cardType]) => (!nodes.length && cardType.type === 'Trigger') || (nodes.length && cardType.type === 'Action')
          ).map(
            ([cardKey, cardValue]) => (
              <Card key={cardKey} draggable className="w-full cursor-grab border-black bg-neutral-100 dark:border-neutral-700 dark:bg-neutral-900" onDragStart={(event) => onDragStart(event, cardKey as EditorCanvasTypes)}>
