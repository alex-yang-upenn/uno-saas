...
import LoadingSvg from '../../workflows/editor/[editorId]/_components/loading-svg'
...
const BillingDashboard = (props: Props) => {
  ...
  useEffect(() => {onStripeProducts()}, [])

  const onPayment = async (id: string) => {
    const { data } = await axios.post( "/api/payment", {priceId: id}, {headers: {"Content-Type": "application/json"}} )
