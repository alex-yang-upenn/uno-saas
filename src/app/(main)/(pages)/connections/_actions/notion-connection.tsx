...
export const onNotionConnect = async (access_token: string, workspace_id: string, workspace_icon: string, workspace_name: string, database_id: string, id: string) => {
  'use server'
  if (access_token) {
    const notion_connected = await db.notion.findFirst({ where: {accessToken: access_token}, include: {connections: {select: {type: true}}} })
    ...
    if (!notion_connected) {
      await db.notion.create({
        data: {
          ...
          connections: {create: {userId: id, type: 'Notion'}}}
        })
    }
  }
}
...
export const onCreateNewPageInDatabase = async (databaseId: string, accessToken: string, content: string) => {
  ...
  const response = await notion.pages.create({
    ...
    properties: { Name: [ {text: {content: content}} ] }
  })
  if (response) {
    return response
  }
