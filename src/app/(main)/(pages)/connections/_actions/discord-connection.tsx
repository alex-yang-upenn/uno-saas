...
export const onDiscordConnect = async (channel_id: string, webhook_id: string, webhook_name: string, webhook_url: string, id: string, guild_name: string, guild_id: string) => {
  if (webhook_id) {
    const webhook = await db.discordWebhook.findFirst({ where: {userId: id}, include: {connections: {select: {type: true}}} })
    ...
    if (!webhook) {
      await db.discordWebhook.create({
        data: {
          ...
          connections: { create: {userId: id, type: 'Discord' },
          }
        }
      })
    }
    ...
    if (webhook) {
      const webhook_channel = await db.discordWebhook.findUnique({ where: {channelId: channel_id}, include: {connections: {select: {type: true}}} })
